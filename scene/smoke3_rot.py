...
parser.add_argument("--log_dir", type=str, default='data/smoke3_rot_f500')
...
parser.add_argument("--circle_radius", type=float, default=0.25)
parser.add_argument("--circle_period", type=float, default=100) # frames
parser.add_argument("--min_frames", type=int, default=0)
parser.add_argument("--max_frames", type=int, default=499)
...
def advect():
	...
	for t in trange(args.num_frames):
		v_path_ = os.path.join(v_path, args.path_format % t)
...
def main():
	...
	for t in trange(args.num_frames, desc='sim'):
		px = 0.5 + args.circle_radius*np.cos(t*2*np.pi/args.circle_period)
		pz = 0.5
		nqx.append(px)
		param_ = [list(nqx)]
		if res_z > 1:
			pz = 0.5 + args.circle_radius*np.sin(t*2*np.pi/args.circle_period)
			nqz.append(pz)
		...
		v_file_path = os.path.join(args.log_dir, 'v', args.path_format % t)
		if res_z > 1:
