...
parser.add_argument("--src_radius", type=float, default=0.16)
...
def main():
	...
	solver = Solver(name='main', gridSize=gs, dim=3)
	...
	omega = solver.create(VecGrid)
	stream = solver.create(VecGrid)
	# vel_out = solver.create(MACGrid)
	...
	for i, (p, pi) in enumerate(zip(p_list,pi_list)):
		...
		source = solver.create(Sphere, center=gs*vec3(p,args.src_y_pos,args.src_z_pos), radius=radius)
		...
		for t in range(args.num_frames):
			...
			# d_file_path = os.path.join(args.log_dir, 'd', args.path_format % (pi, t))
			# np.savez_compressed(d_file_path, 
			# 					x=np.expand_dims(d_, axis=-1),
			# 					y=param_)

			vdb_file_path = os.path.join(args.log_dir, 'vdb', (args.path_format[:-3] % (pi, t))+'vdb')
			...
			# p_file_path = os.path.join(args.log_dir, 'p', args.path_format % (pi, t))
