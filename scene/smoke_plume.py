...
parser.add_argument("--log_dir", type=str, default='data/smoke_plume_f600')
...
num_sim = 600
parser.add_argument("--src_inflow", type=float, default=5)
parser.add_argument("--src_buoyancy", type=float, default=-10e-4)
parser.add_argument("--osc_amp", type=float, default=0.08)
parser.add_argument("--osc_period", type=float, default=30)
...
def test():
	...
	# ax.set_title('wl2 plot (compare to last frame)')

	# # for i in range(600):
	# # 	ax.plot(range(600), wl2_map[i,:])
	# ax.plot(range(600), wl2_map[599,:])
	...
	src_radius = args.resolution_x*args.src_radius
	...
	while True:
		...
		src_x_pos = np.sin(np.pi*2*t/args.osc_period)*args.osc_amp + 0.5
		src_center = gs*vec3(src_x_pos,args.src_y_pos,args.src_z_pos)
...
def generate():
	...
	src_radius = args.resolution_x*args.src_radius
	inflow = vec3(0,args.src_inflow,0)
